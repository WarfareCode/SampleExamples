...
function GetPluginConfig( AgAttrBuilder )
{
	if( m_AgAttrScope == null )
	{
		...
		else if (m_dimension == "DateFormat")
		{
			...
			AgAttrBuilder.AddQuantityDispatchProperty ( m_AgAttrScope, "Tolerance", "Tolerance", "Tolerance", "Seconds", "Seconds", eFlagNone );
		}
		else
		{
			// attr quantity, use internal units that were given
			AgAttrBuilder.AddQuantityDispatchProperty ( m_AgAttrScope, "DesiredValue", "Desired Value", "DesiredValue", m_internalUnit, m_internalUnit, eFlagNone );
			AgAttrBuilder.AddQuantityDispatchProperty ( m_AgAttrScope, "Tolerance", "Tolerance", "Tolerance", m_internalUnit, m_internalUnit, eFlagNone );
		}
	}
