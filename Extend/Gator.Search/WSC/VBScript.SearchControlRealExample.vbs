...
Function GetPluginConfig( AgAttrBuilder )
	...
	If( m_AgAttrScope is Nothing ) Then
		...
		elseif (m_dimension = "DateFormat") then
			...
			Call AgAttrBuilder.AddQuantityDispatchProperty ( m_AgAttrScope, "Step", "Step when searching for bounds", "Step", "Seconds", "Seconds", eFlagNone )
		else
			' attr quantity, use internal units that were given
			Call AgAttrBuilder.AddQuantityDispatchProperty ( m_AgAttrScope, "Step", "Step when searching for bounds", "Step", m_internalUnit, m_internalUnit, eFlagNone )
		End If
