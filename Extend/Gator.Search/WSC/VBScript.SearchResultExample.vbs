...
Function GetPluginConfig( AgAttrBuilder )
	...
	If( m_AgAttrScope is Nothing ) Then
		...
		elseif (m_dimension = "DateFormat") then
			...
			Call AgAttrBuilder.AddQuantityDispatchProperty ( m_AgAttrScope, "Tolerance", "Tolerance", "Tolerance", "Seconds", "Seconds", eFlagNone )
		else
			' attr quantity, use internal units that were given
			Call AgAttrBuilder.AddQuantityDispatchProperty ( m_AgAttrScope, "DesiredValue", "Desired Value", "DesiredValue", m_internalUnit, m_internalUnit, eFlagNone )
			Call AgAttrBuilder.AddQuantityDispatchProperty ( m_AgAttrScope, "Tolerance", "Tolerance", "Tolerance", m_internalUnit, m_internalUnit, eFlagNone )
		End If
