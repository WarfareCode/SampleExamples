...
function GetPluginConfig( AgAttrBuilder )
{
	if( m_AgAttrScope == null )
	{
		...
		else if (m_dimension == "DateFormat")
		{
			...
			AgAttrBuilder.AddQuantityDispatchProperty ( m_AgAttrScope, "Step", "Step when searching for bounds", "Step", "Seconds", "Seconds", eFlagNone );
		}
		else
		{
			// attr quantity, use internal units that were given
			AgAttrBuilder.AddQuantityDispatchProperty ( m_AgAttrScope, "Step", "Step when searching for bounds", "Step", m_internalUnit, m_internalUnit, eFlagNone );
		}
	}
