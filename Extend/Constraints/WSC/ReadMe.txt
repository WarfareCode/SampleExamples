...
The following is a step by step procedure for registering Access Constraint Plugin components.
...
3.  This step registers your component as a COM component.
    ...
    'regsvr32 YOUR_FILENAME'. For example: regsvr32 AGI.AccessConstraint.Plugin.Examples.Stk.VBScript.Example1.Wsc  .
    
    Dll components not built using .NET
    
    Create a command window (under the Start menu, hit "Run ...", type 'Command' and hit OK).
    Change your directory (using cd) to that of the file. At the command prompt, type 
    'regsvr32 YOUR_FILENAME'. For example: regsvr32 AGI.Access.Constraint.Plugin.Examples.Stk.CPP.Examples.dll  .
    ...
    regasm /codebase AGI.Access.Constraint.Plugin.CSharp.GenExample  .
...
4.	This plugin has already been registered in the "Access Constraint Plugins" Category in the
...
7.  Use STK documentation for directions on how to configure an Access Constraint plugin for a 
    given satellite.
