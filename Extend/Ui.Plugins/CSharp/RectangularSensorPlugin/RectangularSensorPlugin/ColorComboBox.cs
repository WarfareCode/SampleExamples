    ...
    public partial class ColorComboBox : UserControl
    {
        ...
        private class ColorComboButton : CheckBox
        {
            ...
            private class ColorPopup : UserControl
            {
                ...
                {
                    // Bug Fix - Color does not update after selected with ColorDialog()
                    //   Temporarily turn off AutoClose so that
                    //   the popup (Parent) does not close when the dialog
                    //   opens and causes it to loose focus, thus preventing
                    //   the popup from knowing the color changed.
                    ((ToolStripDropDown)Parent).AutoClose = false;
                    ...
                    // Turn AutoClose back on since the bug only applied to the ColorDialog()
                    ((ToolStripDropDown)Parent).AutoClose = true;
                }
            }
            #endregion
