...
STDMETHODIMP CExample1::GetPluginConfig(IAgAttrBuilder * pAttrBuilder, IDispatch** ppDispScope)
{
    ...
    try
    {
        if( !m_pDispScope )
        {
            ...
            EXCEPTION_HR(pAttrBuilder->AddQuantityDispatchProperty(m_pDispScope, CComBSTR("ConstantCoefficient"), CComBSTR("ConstantCoefficient"),
                                                                   CComBSTR("ConstantCoefficient"), CComBSTR("dB"), CComBSTR("units"), 0));
