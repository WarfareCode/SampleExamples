...
STDMETHODIMP CExample1::GetPluginConfig(IAgAttrBuilder * pAttrBuilder, IDispatch** ppDispScope)
{
    ...
    try
    {
        if( !m_pDispScope )
        {
            ...
            EXCEPTION_HR(pAttrBuilder->AddQuantityDispatchProperty(m_pDispScope, CComBSTR("PatchArea"), CComBSTR("PatchArea"),
                                                                   CComBSTR("PatchArea"), CComBSTR("sqm"), CComBSTR("sqm"), 0));
            EXCEPTION_HR(pAttrBuilder->AddQuantityDispatchProperty(m_pDispScope, CComBSTR("OffsetAngle"), CComBSTR("OffsetAngle"),
                                                                   CComBSTR("OffsetAngle"), CComBSTR("deg"), CComBSTR("rad"), 0));
        }
