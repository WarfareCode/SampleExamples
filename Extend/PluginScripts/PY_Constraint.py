...
def PY_Constraint_GetAccessList( argList ) :
	...
	global debug
	if debug == 1:
		msg = 'GetAccessList- BaseClass: %s' % (strBaseClass)
		win32ui.MessageBox(msg, 'Python Scripting', 0)
...
def PY_Constraint_compute( argList ) :
	...
	if PY_Constraint_init < 0 :
		...
		if debug == 1 :
			msg = 'numArgs = %d, mode = %s' % (len(argList), str(argList[0]))
			win32ui.MessageBox(msg, 'Python Scripting', 0)
			toPos = argList[PY_Constraint_Inputs['toPos']]
			x = float(toPos[0])
			y = float(toPos[1])
			z = float(toPos[2])
			msg = 'ToPos = (%f,%f,%f)' % (x,y,z)
			win32ui.MessageBox(msg, 'Python Scripting', 0)
