...
implements SelectionListener
{
	...
	private AnimationComposite		m_amc;
	...
	throws AgException
	{
		...
		this.m_Shell.setText("CustomApp_SWT_STK_X_Animation");
		this.m_Shell.setSize(900, 500);
		...
		this.m_amc = new AnimationComposite(this.m_Shell, SWT.NONE);
		this.m_amc.setLayoutData(new GridData(SWT.CENTER, SWT.BOTTOM, false, false, 1, 1));
		this.m_amc.addSelectionListener(this);
	...
	implements Listener
	{
		public void handleEvent(Event e)
		{
			...
			try
			{
				IAgStkObject scenObj = m_AgStkObjectRoot.getCurrentScenario();
				if(scenObj != null)
				{
					agi.stkutil.IAgExecCmdResult result = m_AgStkObjectRoot.executeCommand("GetReport * \"InstallInfoCon\"");
	...
	implements Listener
	{
		public void handleEvent(Event e)
		{
			...
			StringBuffer msg = new StringBuffer();
			msg.append("Name:\t\t\tCustomApp_SWT_STK_X_Animation");
			msg.append("\n");
	...
	public void widgetSelected(SelectionEvent e)
	{
		try
		{
			...
			if(parent == this.m_amc.m_AnimPlayForwardButton)
			{
				this.m_AgStkObjectRoot.playForward();
			}
			else if(parent == this.m_amc.m_AnimPlayBackwardButton)
			{
				this.m_AgStkObjectRoot.playBackward();
			}
			else if(parent == this.m_amc.m_AnimPauseButton)
			{
				this.m_AgStkObjectRoot.pause();
			}
			else if(parent == this.m_amc.m_AnimFasterButton)
			{
				this.m_AgStkObjectRoot.faster();
			}
			else if(parent == this.m_amc.m_AnimSlowerButton)
			{
				this.m_AgStkObjectRoot.slower();
			}
			else if(parent == this.m_amc.m_AnimStepBackwardButton)
			{
				this.m_AgStkObjectRoot.stepBackward();
			}
			else if(parent == this.m_amc.m_AnimStepForwardButton)
			{
				this.m_AgStkObjectRoot.stepForward();
			}
			else if(parent == this.m_amc.m_AnimRewindButton)
			{
				this.m_AgStkObjectRoot.rewind();
			}
		}
		catch(Throwable t)
		{
	...
	private void newScenario(String name)
	{
		try
		{
			m_AgStkObjectRoot.closeScenario();
			m_AgStkObjectRoot.newScenario(name);
			IAgStkObject scenObject = m_AgStkObjectRoot.getCurrentScenario();
