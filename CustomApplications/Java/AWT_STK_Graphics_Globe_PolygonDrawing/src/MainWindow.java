...
extends CustomApplicationSTKEngineSampleBaseJFrame
{
	...
	private final static String					s_TITLE				= "CustomApp_AWT_STK_Graphics_Globe_PolygonDrawing";
	...
	private IAgStkGraphicsTriangleMeshPrimitive	m_editingPolygon;
	...
	throws AgCoreException
	{
		...
		if(scenario != null)
		{
			...
			if(this.m_lastMousePosition != null)
			{
				Object[] position = new Object[2];
				position[0] = new Double(this.m_lastMousePosition.getX());
				position[1] = new Double(this.m_lastMousePosition.getY());
	...
	throws AgCoreException
	{
		...
		if(scenario != null)
		{
			...
			if(this.m_editingPoints.length > 2)
			{
				IAgStkGraphicsFactoryAndInitializers initrs = manager.getInitializers();
				IAgStkGraphicsSurfacePolygonTriangulatorInitializer spt = initrs.getSurfacePolygonTriangulator();
				IAgStkGraphicsSurfaceTriangulatorResult result = spt.computeCartographic("Earth", this.m_editingPoints);
				IAgStkGraphicsTriangleMeshPrimitiveFactory tmp = initrs.getTriangleMeshPrimitive();
				IAgStkGraphicsTriangleMeshPrimitive polygon = tmp.initializeWithSetHint(AgEStkGraphicsSetHint.E_STK_GRAPHICS_SET_HINT_INFREQUENT);

				IAgStkGraphicsPrimitive polygonPrim = (IAgStkGraphicsPrimitive)polygon;
				IAgStkGraphicsPrimitive editPrim = (IAgStkGraphicsPrimitive)this.m_editingPolygon;
				polygonPrim.setColor(editPrim.getColor());
				...
				primitives.add((IAgStkGraphicsPrimitive)polygon);

				Object[] closedPoints = new Object[this.m_editingPoints.length + 3];
				for(int i = 0; i < this.m_editingPoints.length; i++)
				{
					closedPoints[i] = this.m_editingPoints[i];
				}
				int cpLength = closedPoints.length;
				closedPoints[cpLength - 3] = this.m_editingPoints[0];
				closedPoints[cpLength - 2] = this.m_editingPoints[1];
				closedPoints[cpLength - 1] = this.m_editingPoints[2];
				...
				((IAgStkGraphicsPrimitive)newPolyline).setColor(((IAgStkGraphicsPrimitive)m_editingPolygon).getColor());
				newPolyline.setWidth(2);
				newPolyline.setCartographic("Earth", closedPoints);
			...
			((IAgStkGraphicsPrimitive)m_editingPolygon).setColor(AgAwtColorTranslator.fromAWTtoCoreColor(color));
	...
	throws AgCoreException
	{
		if(m_editingPoints.length > 6)
		{
			try
			{
				IAgStkGraphicsSceneManager manager = ((IAgScenario)root.getCurrentScenario()).getSceneManager();
				IAgStkGraphicsSurfaceTriangulatorResult result = manager.getInitializers().getSurfacePolygonTriangulator().computeCartographic("Earth", positions);
				m_editingPolygon.setTriangulator((IAgStkGraphicsTriangulatorResult)result);
			}
			catch(Throwable t)
			{
		...
		else if(m_editingPoints.length == 0)
		{
			Object[] pos = new Object[0];
			Object[] nor = new Object[0];
			Object[] ind = new Object[0];
			m_editingPolygon.set(pos, nor, ind);
		}
	...
	throws AgCoreException
	{
		...
		m_editingPolygon = manager.getInitializers().getTriangleMeshPrimitive().initializeDefault();
		((IAgStkGraphicsPrimitive)m_editingPolygon).setColor(AgAwtColorTranslator.fromAWTtoCoreColor(color));
		manager.getPrimitives().add((IAgStkGraphicsPrimitive)m_editingPolygon);
