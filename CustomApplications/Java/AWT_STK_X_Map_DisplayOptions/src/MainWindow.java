...
implements ActionListener
{
	...
	private final static String		s_TITLE				= "CustomApp_AWT_STK_X_Map_DisplayOptions";
	...
	throws Throwable
	{
		...
		long coreColor = this.m_AgMapCntrlClass.getBackColor();
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		try
		{
			...
			{
				long oldCoreColor = this.m_AgMapCntrlClass.getBackColor();
				...
				this.m_AgMapCntrlClass.setBackColor(newCoreColor);
			...
			else if(this.m_SampleJPanel.getSplashJPanel().isUpdateJButton(src))
			{
				...
				if(showSplash)
				{
					if(this.m_SampleJPanel.getSplashJPanel().getUseUserDefinedSplashScreen())
					{
						...
						this.m_AgMapCntrlClass.setPictureFromFile(splashFilePath);
					}
					else
					{
						this.m_AgMapCntrlClass.setPictureFromFile("");
					}
				}
				this.m_AgMapCntrlClass.setNoLogo(!showSplash);
			}
			else if(this.m_SampleJPanel.getProgressImageJPanel().isUpdateJButton(src))
			{
				...
				{
					this.m_AgMapCntrlClass.setShowProgressImage(AgEShowProgressImage.E_SHOW_PROGRESS_IMAGE_NONE);
				}
				else if(this.m_SampleJPanel.getProgressImageJPanel().getUseDefaultProgressImageFile())
				{
					this.m_AgMapCntrlClass.setShowProgressImage(AgEShowProgressImage.E_SHOW_PROGRESS_IMAGE_DEFAULT);
				}
				else if(this.m_SampleJPanel.getProgressImageJPanel().getUseUserDefinedProgressImageFile())
				{
					this.m_AgMapCntrlClass.setShowProgressImage(AgEShowProgressImage.E_SHOW_PROGRESS_IMAGE_USER);

					int xOffset = this.m_SampleJPanel.getProgressImageJPanel().getXOffset();
					this.m_AgMapCntrlClass.setProgressImageXOffset(xOffset);
					
					int yOffset = this.m_SampleJPanel.getProgressImageJPanel().getYOffset();
					this.m_AgMapCntrlClass.setProgressImageYOffset(yOffset);
	
					int xOrigin = this.m_SampleJPanel.getProgressImageJPanel().getXOrigin();
					this.m_AgMapCntrlClass.setProgressImageXOrigin(xOrigin);
					
					int yOrigin = this.m_SampleJPanel.getProgressImageJPanel().getYOrigin();
					this.m_AgMapCntrlClass.setProgressImageYOrigin(yOrigin);
					...
					this.m_AgMapCntrlClass.setProgressImageFile(progressImageFilePath);
				}
			}
		}
		catch(Throwable t)
