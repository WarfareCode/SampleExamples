...
public final class Main
{
	...
	private AgGlobeCntrlEventsHandler 	m_AgGlobeCntrlEventsHandler;
	...
	throws AgException
	{
		...
		this.m_Shell.setText("CustomApp_SWT_STK_X_Globe_Events");
		...
		this.m_AgGlobeCntrlEventsHandler = new AgGlobeCntrlEventsHandler();
		this.m_AgGlobeCntrlClass.addIAgGlobeCntrlEvents(this.m_AgGlobeCntrlEventsHandler);
	}
	...
	implements Listener
	{
		public void handleEvent(Event e)
		{
			...
			msg.append("\n");
			msg.append("2. Interact with the globe and notice the events output");
			msg.append("\n");
	...
	implements Listener
	{
		public void handleEvent(Event e)
		{
			...
			StringBuffer msg = new StringBuffer();
			msg.append("Name:\t\t\tCustomApp_SWT_STK_X_Globe_Events");
			msg.append("\n");
	...
	implements DisposeListener
	{
		public void widgetDisposed(DisposeEvent e)
		{
			try
			{
				Main.this.m_AgGlobeCntrlClass.removeIAgGlobeCntrlEvents(Main.this.m_AgGlobeCntrlEventsHandler);
	...
	class AgGlobeCntrlEventsHandler 
	implements IAgGlobeCntrlEvents
	{
		public void onAgGlobeCntrlEvent(AgGlobeCntrlEvent evt)
		{
			try
			{
				...
				{
					System.out.println("Click " + sb.toString());
				}
				else if(type == AgGlobeCntrlEvent.TYPE_DBL_CLICK)
				{
					System.out.println("DblClick " + sb.toString());
				}
				else if(type == AgGlobeCntrlEvent.TYPE_KEY_UP)
				{
					System.out.println("KeyUp " + sb.toString());
				}
				else if(type == AgGlobeCntrlEvent.TYPE_KEY_PRESS)
				{
					System.out.println("KeyPress " + sb.toString());
				}
				else if(type == AgGlobeCntrlEvent.TYPE_KEY_DOWN)
				{
					System.out.println("KeyDown " + sb.toString());
				}
				else if(type == AgGlobeCntrlEvent.TYPE_MOUSE_UP)
				{
					System.out.println("MouseUp " + sb.toString());
				}
				else if(type == AgGlobeCntrlEvent.TYPE_MOUSE_MOVE)
				{
					System.out.println("MouseMove " + sb.toString());
					Integer x = (Integer)params[2];
					Integer y = (Integer)params[3];

					IAgPickInfoData pickInfoData = Main.this.m_AgGlobeCntrlClass.pickInfo(x.intValue(), y.intValue());
					...
					{
						System.out.println("\tObjectPath=" + objPath);
					}
